...
impl<'a> SemanticAnalyzer<'a> {
    pub fn infer_expr_type(
        ...
        let ty = match self.ast.expr(expr_id) {
            ...
            Expr::Binary { op, lhs, rhs } => {
                self.infer_binary(*op, *lhs, *rhs, expected_type, expr_id)
            }
            ...
            Expr::Slice {
                ...
                left,
                right,
            } => self.infer_slice(expected_type, *target, *left, *right, expr_id),
            Expr::RecordAccess { target, field } => {
                self.infer_record_access(*target, *field, expr_id)
            }
    ...
    fn infer_literal(
        ...
        let text = self.symbols.interner.get(name);
